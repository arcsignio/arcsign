...
## [v1.6.3] — 2026-09-02 — 修正金額顯示

這版沒有新功能，全部是把顯示錯的數字改對。

### Fixed

- **交易速度選項全部顯示相同的手續費** — 在 BNB Chain 上，慢／普通／快三個
  選項都顯示「<0.0001 BNB」，等於無從比較。BSC 的手續費本來就那麼小
  （一次轉帳約 0.00002 BNB），而畫面套用了「餘額」的顯示規則，把小於
  0.0001 的值一律收合。現在會顯示完整數字，三個選項可以比較。

- **授權管理裡認得出的協議名稱看不見** — 「你授權給 Uniswap」這行字用的
  顏色在白底上幾乎透明。諷刺的是**只有能識別出來的協議會消失**，不明地址
  反而看得清楚。

- **授權額度算錯，USDC/USDT 差 10^12 倍** — 這兩種代幣的小數位是 6 而非
  一般的 18，但畫面一律當成 18。**一筆 100 USDC 的授權顯示成「<0.001 USDC」**
  ——該撤銷的東西被呈現成不值得撤銷。額度的量級標籤也錯：一百萬顆被標成
  「>1T」，差了一百萬倍。

- **dApp 名稱與網址被截斷** — 兩者是判斷要不要簽章的唯一依據，而截斷會把
  `uniswap.org.evil.io` 顯示成 `uniswap.org…`，正好藏掉能分辨真假的那一段。
  現在完整換行顯示。

- **手續費在某些情況顯示「NaN」** — 送出前的手續費欄位會出現這三個字母。

- **同一個零有兩種寫法** — WalletConnect 批准提示裡，同樣是零的金額有時
  顯示「0 ETH」、有時「0.000000 ETH」。

- **質押的 TVL 虛報三分之一** — 15 億美元的 TVL 顯示成「$2M」（1.5 進位
  成 2）。TVL 為零時整列消失而非顯示 $0。

- **同一筆金額在不同畫面顯示成不同結果** — 餘額 1234.5678 在錢包頁是
  `1234.5678`、質押頁是 `1234.57`、DeFi 頁是 `1,234.57`。使用非英文系統
  語言時更亂：某些畫面看到 `1.234,57`、某些看到 `1234.5678`，而這兩種寫法
  在不同慣例下差一千倍。現在全部統一為 `1,234.5678`。

- **模擬的餘額變化對特殊代幣算錯** — 小數位為 0 的代幣被當成 18 位，
  一整個一百萬顯示成「<0.0001」。

- 利率（APY）顯示到小數第五、六位且彼此寬度不一，現在統一兩位。

### Changed

- 金額一律**無條件捨去**而非四捨五入。顯示 1.5 就保證至少有 1.5——四捨五入
  會把 1.4999 顯示成 1.5，照著輸入轉帳會失敗。
- 千分位固定使用英文慣例，不再隨系統語言變動。
