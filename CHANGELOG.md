...
- **強制刪除的名稱比對改為忽略大小寫與前後空格** — 實機測試時，使用者
  打自己的錢包名 `test2` 卻被拒絕：macOS 會自動把文字欄位第一個字母
  大寫，實際送出的是 `Test2`。

  錢包名稱沒有唯一性檢查（兩個錢包可以同名），所以「完全相符」從來就
  不是識別錢包的東西——`walletID` 才是。名稱只確認使用者知道這是哪個
  錢包，折疊大小寫不影響這件事；嚴格比對沒換到安全，只換到「使用者
  打對卻被擋」。打成**不同的**名字仍然拒絕，這半邊才是真正擋住
  「刪到隔壁那個錢包」的部分。

- 名稱不符時只有灰按鈕、沒有任何說明，看起來像 app 當機；現在欄位
  下方直接顯示原因。
- 確認欄位加上 `autoCapitalize="none"` / `autoCorrect="off"`，從源頭
  關掉作業系統的自動大寫。
- 強制模式多出警告框後，刪除對話框在較矮的視窗會超出畫面、底部連結
  被切掉；改為最大高度 90vh 並可捲動。

### Known limitations

- 強制刪除的**限流行為尚未實機驗證**。單元測試涵蓋密碼驗證與名稱比對，
  但「連續輸錯後被暫時鎖定」這條路徑目前只有程式碼層面的保證
  （與 `UnlockApp` 共用 `appRateLimiter`），未在實機上跑過。
