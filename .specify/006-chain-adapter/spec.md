...
3. **Given** a transaction with memo field, **When** building for Cosmos, **Then** includes memo in transaction, **When** building for Bitcoin, **Then** gracefully handles unsupported memo
...
**Independent Test**: Can be tested by deriving addresses for multiple chains (BTC, ETH, ATOM) from the same BIP39 mnemonic and verifying correct derivation paths and address formats.
...
1. **Given** an Ethereum chain adapter, **When** querying capabilities, **Then** returns {supportsEIP1559: true, supportsMemo: false, supportsMultiSig: true}
2. **Given** a Cosmos chain adapter, **When** querying capabilities, **Then** returns {supportsMemo: true, supportsFeeDelegation: true}
3. **Given** a legacy chain adapter, **When** querying unsupported capability, **Then** returns false without error
...
- **FR-013**: Sign method MUST support offline signing by accepting pre-built unsigned transactions
- **FR-014**: Sign method MUST support external signers (hardware wallets) through abstraction interface
- **FR-015**: Sign method MUST preserve auditable signing payload (human-readable and binary forms)
- **FR-016**: Sign method MUST validate signatures against expected public keys before returning

#### Transaction Broadcasting

- **FR-017**: Broadcast method MUST be idempotent (broadcasting same signed tx multiple times returns same result)
- **FR-018**: Broadcast method MUST return submission receipt with transaction hash/ID
- **FR-019**: Broadcast method MUST provide status query hooks for checking transaction confirmation
- **FR-020**: Broadcast method MUST detect and report already-broadcast or already-confirmed transactions

#### Address Derivation

- **FR-021**: Derive method MUST support BIP39 mnemonic as key source
- **FR-022**: Derive method MUST support extended public keys (xpub) as key source
- **FR-023**: Derive method MUST support hardware wallet abstraction as key source
- **FR-024**: Derive method MUST return chain-specific addresses with proper encoding (P2WPKH, checksummed 0x, bech32, etc.)
- **FR-025**: Derive method MUST use chain-appropriate BIP44/SLIP44 derivation paths

#### Error Handling and Classification

- **FR-026**: System MUST classify errors into three categories: Retryable, NonRetryable, UserIntervention
- **FR-027**: Retryable errors MUST include: temporary network failures, RPC timeouts, nonce conflicts (for automatic retry)
- **FR-028**: NonRetryable errors MUST include: invalid transaction format, insufficient balance, unsupported operation
- **FR-029**: UserIntervention errors MUST include: insufficient fee during congestion, multi-sig approval needed, hardware wallet approval needed
- **FR-030**: Each error MUST include human-readable message, error code, and retry guidance

#### Capability Detection and Versioning

- **FR-031**: Each chain adapter MUST expose Capabilities() method returning supported features
- **FR-032**: Capabilities MUST include flags for: EIP-1559 support, memo field support, multi-signature support, fee delegation support
- **FR-033**: Adapters MUST declare interface version for backward compatibility
- **FR-034**: System MUST gracefully handle adapters with different capability sets

#### Testing and Observability

- **FR-035**: All adapter methods MUST support dependency injection of RPC clients for unit testing
- **FR-036**: All adapter methods MUST support dependency injection of signers for unit testing
- **FR-037**: System MUST expose observable metrics: RPC response times, failure rates, chain health status
- **FR-038**: System MUST provide timing instrumentation for each adapter method call

#### Extensibility

- **FR-039**: Adding a new chain MUST only require: creating new adapter implementation and minimal configuration
- **FR-040**: Adding a new chain MUST NOT require changes to: UI layer, service layer, or other adapters
- **FR-041**: Adapter registry MUST support dynamic adapter loading and chain selection
...
- **Capabilities**: Feature flags indicating what a chain adapter supports (EIP-1559, memo, multi-sig, etc.)
