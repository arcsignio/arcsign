...
- 💱 **Token Swap** - 整合 1inch DEX 聚合器，支援跨 DEX 最佳價格路由
...
- **自動鎖定**: 15 分鐘閒置自動登出
- **密碼驗證**: 每筆交易需重新驗證
- **敏感資料清理**: 私鑰僅存在記憶體 ~50-100ms
- **截圖保護**: 可選的截圖防護功能

### 支援網路

| 網路 | Chain ID | 代幣查詢 | 交易發送 | Swap |
|------|----------|----------|----------|------|
...
Token Swap 功能整合 1inch DEX 聚合器，提供跨多個去中心化交易所的最佳價格路由。

### 功能特點

- **最佳價格路由**: 自動比較 Uniswap、SushiSwap、Curve 等多個 DEX
...
- **私鑰安全**: 私鑰僅在簽名時短暫存在記憶體 (~50-100ms)
- **密碼驗證**: 每筆交易需要重新輸入錢包密碼
- **滑點保護**: 實際價格超過滑點容忍度時交易會失敗
- **合約驗證**: 僅與官方 1inch Router 合約互動
...
- [ ] 交易歷史記錄查詢 (Alchemy API)
- [ ] NFT 資產顯示
- [ ] DeFi 協議整合 (Aave, Compound)
- [ ] 更多 DEX 聚合器 (0x, Paraswap)
